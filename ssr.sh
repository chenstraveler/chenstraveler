...
#	Version: 1.0.00
#	Author: chenstraveler
#	Blog: http://chenstraveler.pixnet.net/blog
#=================================================
...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} 當前帳號非ROOT(或沒有ROOT權限)，無法繼續操作，請使用${Green_background_prefix} sudo su ${Font_color_suffix}來獲取臨時ROOT權限（執行後會提示輸入當前帳號的密碼）。" && exit 1
}
check_sys(){
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		ss_ssr_determine
		clear && echo "===================================================" && echo
		echo -e " ShadowsocksR帳號 配置信息：" && echo
...
Set_config_port(){
	while true
	do
	echo -e "請輸入要設置的ShadowsocksR帳號 端口"
...
Set_config_obfs(){
 ...
 另外, 如果你選擇了 tls1.2_ticket_auth，那麼客戶端可以選擇 tls1.2_ticket_fastauth，這樣即能偽裝又不會增加延遲 !
 如果你是在日本、美國等熱門地區搭建，那麼選擇 plain 混淆 可能被牆機率更低 !" && echo
...
Set_config_protocol_param(){
	while true
	do
	echo -e "請輸入要設置的ShadowsocksR帳號 欲限制的設備數 (${Green_font_prefix} auth_* 系列協議 不兼容原版才有效 ${Font_color_suffix})"
	echo -e "${Tip} 設備數限制：每個端口同一時間能連接的客戶端數量(多端口模式，每個端口都是獨立計算)，建議最少2個。"
...
Download_SSR(){
	...
	[[ ! -e "manyuser.zip" ]] && echo -e "${Error} ShadowsocksR服务端 壓縮包 下載失敗 !" && rm -rf manyuser.zip && exit 1
	unzip "manyuser.zip"
	[[ ! -e "/usr/local/shadowsocksr-manyuser/" ]] && echo -e "${Error} ShadowsocksR服務器 解壓失敗 !" && rm -rf manyuser.zip && exit 1
...
Install_SSR(){
	...
	echo -e "${Info} 開始設置 ShadowsocksR帳號配置..."
	Set_config_all
...
debian_View_user_connection_info(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		user_list_all="端口: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 連接IP總數: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t 當前連接IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
		user_IP=""
		echo -e "當前模式: ${Green_background_prefix} "${now_mode}" ${Font_color_suffix} 連接IP總數: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix}"
		...
		do
			...
			user_list_all=${user_list_all}"端口: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 連接IP總數: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t 當前連接IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
			user_IP=""
		done
		echo -e "當前模式: ${Green_background_prefix} "${now_mode}" ${Font_color_suffix} 用戶總數: ${Green_background_prefix} "${user_total}" ${Font_color_suffix} 連接IP總數: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix} "
		echo -e "${user_list_all}"
	fi
}
...
Port_mode_switching(){
	SSR_installation_status
	if [[ -z "${now_mode}" ]]; then
		echo && echo -e "	: ${Green_font_prefix}單端口${Font_color_suffix}" && echo
...
1. 安裝開啟BBR，需要更換內核，存在更換失敗等風險(重啟後無法重新開機)
2. 本腳本僅支持 Debian / Ubuntu 系統更換內核，OpenVZ和Docker 不支持更換內核
3. Debian 更換內核過程中會提示 [ 是否中止卸載內核 ] ，請選擇 ${Green_font_prefix} NO ${Font_color_suffix}" && echo
...
————————————
  ...
  ——說明：SSR默認只輸出錯誤日誌，此項可切換輸出詳細的訪問日誌" && echo
...
Update_Shell(){
	...
	echo -e "腳本已更新為最新版本[ ${sh_new_ver} ] !(注意：因為更新方式為直接覆蓋當前運行的腳本，所以可能下面會提示一些報錯，無視即可)" && exit 0
}
# 顯示 菜單狀態
menu_status(){
	if [[ -e ${config_user_file} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
			echo -e " 當前狀態: ${Green_font_prefix}已安裝${Font_color_suffix} 并 ${Green_font_prefix}已啟動${Font_color_suffix}"
		else
...
————————————
  ${Green_font_prefix}5.${Font_color_suffix} 查看 帳號信息
